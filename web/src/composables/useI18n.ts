...
const zh: Record<string, string> = {
  ...
  'skill.cliMethod': '命令行',
  'skill.agentHint': '复制给你的 AI 助手，由它完成安装。',
  'skill.cliHint': '依次执行以下命令，安装当前浏览版本。',
  'skill.copyPrompt': '复制安装 Prompt',
  'skill.copyCommands': '复制全部命令',
  'skill.agentPrompt': '请帮我安装这个 Skill 到当前项目适合你的技能目录。技能页面：',
  'skill.agentPromptEnd': '先检查 Node.js 和 skb 是否可用，已有 CLI 可跳过 npm 安装。使用上述站点和指定版本；如需登录，请引导我完成，不要索取密码或令牌。根据当前 Agent 用 skb install --help 确认安装目录参数，安装后确认 SKILL.md 的实际路径。',
...
const en: Record<string, string> = {
  ...
  'skill.agentHint': 'Copy to your AI assistant to install this skill.',
  'skill.cliHint': 'Run these commands to install the selected version.',
  ...
  'skill.agentPromptEnd': 'Check Node.js and skb first; skip npm installation if the CLI is available. Use the site and version above. Guide me through login if required without requesting passwords or tokens. Use skb install --help to select the directory for this agent, then verify the installed SKILL.md path.',
